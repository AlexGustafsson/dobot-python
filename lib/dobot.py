import serial
import threading
from time import sleep

from message import Message


class Dobot:
    def __init__(self, port):
        threading.Thread.__init__(self)
        self.lock = threading.Lock()

        self.serial = serial.Serial(
            port=port,
            baudrate=115200,
            parity=serial.PARITY_NONE,
            stopbits=serial.STOPBITS_ONE,
            bytesize=serial.EIGHTBITS
        )

    def send(self, message):
        self.lock.acquire()
        self.serial.write(message)
        self.lock.release()

    def connected(self):
        return self.serial.isOpen()

    def get_device_serial_number(self):
        request = Message([0xAA, 0xAA], 2, 0, 0, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def set_device_serial_number(self, serial_number):
        request = Message([0xAA, 0xAA], 2, 0, 1, 0, [serial_number])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def get_device_name(self):
        request = Message([0xAA, 0xAA], 2, 1, 0, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def set_device_name(self, device_name):
        request = Message([0xAA, 0xAA], 2, 1, 1, 0, [device_name])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def get_device_version(self):
        request = Message([0xAA, 0xAA], 2, 2, 0, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def get_pose(self):
        request = Message([0xAA, 0xAA], 2, 10, 0, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def home(self, wait=True):
        is_queued = 1 if wait else 0
        request = Message([0xAA, 0xAA], 2, 31, 1, is_queued, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def move_to(self, x, y, z, r, wait=True, mode=2):
        is_queued = 1 if wait else 0
        request = Message([0xAA, 0xAA], 2, 84, 1, is_queued, [mode, x, y, z, r])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def move_to_relative(self, x, y, z, r, wait=True):
        return self.move_to(x, y, z, r, wait, 8)

    def get_joint_paramaters(self):
        request = Message([0xAA, 0xAA], 2, 83, 0, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def set_joint_parameters(self, velocity_ratio, acceleration_ratio, wait=True):
        if acceleration_ratio <= 0:
            return None

        is_queued = 1 if wait else 0
        request = Message([0xAA, 0xAA], 2, 83, 1, is_queued, [velocity_ratio, acceleration_ratio])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def get_laser_state(self):
        request = Message([0xAA, 0xAA], 2, 61, 0, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def set_laser_state(self, enable_control, enable_laser, wait=True):
        is_queued = 1 if wait else 0
        request = Message([0xAA, 0xAA], 2, 61, 1, is_queued, [enable_control, enable_laser])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def enable_laser(self, wait=True):
        return self.set_laser_state(True, True, wait)

    def disable_laser(self, wait=True):
        return self.set_laser_state(False, False, wait)

    def move_in_circle(self, start_point, end_point, wait=True):
        is_queued = 1 if wait else 0
        request = Message([0xAA, 0xAA], 2, 101, 1, is_queued, start_point + end_point)
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def get_queue_index(self):
        request = Message([0xAA, 0xAA], 2, 246, 0, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def start_queue(self):
        request = Message([0xAA, 0xAA], 2, 240, 1, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def stop_queue(self):
        request = Message([0xAA, 0xAA], 2, 242, 1, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def clear_queue(self):
        request = Message([0xAA, 0xAA], 2, 245, 1, 0, [])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def wait(self):
        queue_index = self.get_queue_index()
        while True:
            current_queue_index = self.get_queue_index()
            if current_queue_index is not None and current_queue_index != queue_index:
                break
            sleep(0.1)

    def move_to_continous(self, x, y, z, wait=True):
        is_queued = 1 if wait else 0
        request = Message([0xAA, 0xAA], 2, 91, 1, is_queued, [1, x, y, z, 0])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params

    def set_continous_joint_parameters(self, max_planned_acceleration, max_junction_velocity, period, wait=True):
        is_queued = 1 if wait else 0
        request = Message([0xAA, 0xAA], 2, 90, 1, is_queued, [max_planned_acceleration, max_junction_velocity, period, 0])
        self.send(request.package())
        response = Message.read(self.serial)
        return response.params
